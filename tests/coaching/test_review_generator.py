...
class ReviewGeneratorTest(unittest.TestCase):
    ...
    def test_events_are_sorted_by_selected_impact_magnitude(self) -> None:
        low = make_verified_event(
            "fork_missed",
            eval_delta=-50,
            eval_delta_for_event_side=-100,
            impact_magnitude=100,
        )
        high = make_verified_event(
            "hanging_piece_lost",
            eval_delta=-200,
            eval_delta_for_event_side=-200,
            impact_magnitude=200,
            move_uci="g1f3",
        )
    ...
    def test_impact_magnitude_takes_precedence_over_raw_eval_delta(self) -> None:
        raw_eval_larger = make_verified_event(
            "fork_missed",
            eval_delta=-500,
            eval_delta_for_event_side=-25,
            impact_magnitude=25,
        )
        impact_larger = make_verified_event(
            "hanging_piece_lost",
            eval_delta=-50,
            eval_delta_for_event_side=-150,
